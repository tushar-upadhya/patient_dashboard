...
import { Skull, StethoscopeIcon, UserMinus, UsersIcon } from "lucide-react";

export function Overview() {
    const { admissions, discharges, mortality, emergencies } =
        useAppSelector(selectFilteredData);

    const totalAdmissions = admissions.reduce(
        (sum, item) => sum + item.count,
        0
    );
    const totalDischarges = discharges.reduce(
        (sum, item) => sum + item.count,
        0
    );
    const totalMortality = mortality.reduce((sum, item) => sum + item.count, 0);
    const totalEmergencies = emergencies.reduce(
        (sum, item) => sum + item.count,
        0
    );
